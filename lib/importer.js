...
async function writeImportLog(sheets, spreadsheetId, existingLog, rows) {
  if (!rows.length) return;
  const idToRow = new Map();
  existingLog.slice(1).forEach((row, index) => {
    const id = String(row[0] || "");
    if (id) idToRow.set(id, index + 2);
  });
  const updates = [];
  const appends = [];
  rows.forEach(row => {
    const rowNumber = idToRow.get(String(row[0] || ""));
    if (rowNumber) updates.push({ range: `${IMPORT_TAB}!A${rowNumber}:F${rowNumber}`, values: [row] });
  ...
  await appendImportLog(sheets, spreadsheetId, appends);
}
...
async function syncFastmail({ fetchEmails = fetchRecentEmails, sheets = client("https://www.googleapis.com/auth/spreadsheets"), spreadsheetId = sheetId() } = {}) {
  ...
  await writeImportLog(sheets, spreadsheetId, log, logRows);

  return {
    scanned: emails.length,
    mailAccountsChecked: Number(fetchedEmails.mailAccountsChecked) || 1,
    mailboxMessagesChecked: Number(fetchedEmails.mailboxMessagesChecked) || fetchedEmails.length,
...
module.exports = { applyPayments, currencySymbol, idAppears, mergeRecords, syncFastmail, writeImportLog, writeUpsert };
