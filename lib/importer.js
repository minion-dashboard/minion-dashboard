...
function mergeRecords(messages) {
  ...
  messages.forEach(({ record }) => {
    ...
    Object.keys(record).forEach(key => {
      if (record[key] !== "" && record[key] != null) existing[key] = record[key];
    });
...
async function upsertOrders(sheets, spreadsheetId, messages) {
  const records = mergeRecords(messages);
  return writeUpsert(sheets, spreadsheetId, "Orders", 8, 11, records, record => [
    record.event || "", record.date || "", record.venue || "", record.section || "",
    record.row || "", record.seats || "", record.qty || "", record.cost || "",
    record.order_id || "", record.account || "", record.status || ""
  ], (next, current) => {
...
async function syncFastmail({ fetchEmails = fetchRecentEmails, sheets = client("https://www.googleapis.com/auth/spreadsheets"), spreadsheetId = sheetId() } = {}) {
  ...
  const emails = (await fetchEmails({ days })).filter(email => !completedIds.has(String(email.id)));
  ...
  return {
    ...
    imported: groups.lysted.length + groups.viagogo.length + groups.ticketmaster.length + matchedPayments.size,
...
module.exports = { applyPayments, idAppears, mergeRecords, syncFastmail, writeUpsert };
